...
function isWaitingForService(order) {
  // Zolang de bestelling nog niet volledig bezorgd/geserveerd is,
  // blijft deze in de gezamenlijke wachtrij staan.
  return !!order && ['nieuw', 'bereiden', 'klaar'].includes(order.status);
}

// Gebruik uitsluitend de expliciete orderGroupId om gesplitste keuken/bar-
// tickets aan elkaar te koppelen. We mogen NOOIT op device + tafel + tijd
// groeperen: twee losse bestellingen kunnen namelijk dezelfde tafel hebben en
// zelfs exact dezelfde timestamp. Daardoor zouden meerdere echte bestellingen
// ten onrechte als één bestelling worden geteld.
function getOrderGroupId(id, order) {
  return order?.orderGroupId || id;
}

function getOrderGroups() {
  const phaseRank = { nieuw: 0, bereiden: 1, klaar: 2, bezorgd: 3 };
  const groups = new Map();

  Object.entries(allOrders).forEach(([oid, o]) => {
    if (!o) return;

    // Nieuwe selfservice-bestellingen die uit keuken + bar bestaan hebben
    // dezelfde expliciete orderGroupId. Bestellingen zonder groupId zijn
    // bewust individuele bestellingen.
    const gid = getOrderGroupId(oid, o);
    if (!groups.has(gid)) groups.set(gid, { tijd: o.tijd || 0, orders: [] });

    const g = groups.get(gid);
    g.orders.push({ id: oid, order: o });
    if (!g.tijd || (o.tijd || 0) < g.tijd) g.tijd = o.tijd || 0;
  });

  groups.forEach(g => {
    const active = g.orders.filter(x => phaseRank[x.order.status] != null);
    if (!active.length) {
      g.status = 'bezorgd';
      return;
    }

    // Voor de gezamenlijke bestelling is de minst ver gevorderde ticket
    // bepalend. Keuken klaar + bar bereiden = bestelling is nog bereiden.
    g.status = active.reduce((lowest, x) =>
      phaseRank[x.order.status] < phaseRank[lowest] ? x.order.status : lowest,
      active[0].order.status
    );
  });
...
function positionBefore(id, order) {
  ...
  const mijnGroep = getOrderGroupId(id, order);
  const mijn = groups.get(mijnGroep);
  if (!mijn) return 0;

  const mijnTijd = mijn.tijd || order.tijd || 0;
  let voorMij = 0;

  // Eén gezamenlijke wachtrij voor alle fases. Een oudere bestelling telt dus
  // ook mee wanneer die al in 'bereiden' of 'klaar' staat. Alleen een volledig
  // bezorgde bestelling valt uit de wachtrij.
  groups.forEach((g, gid) => {
    if (gid === mijnGroep) return;
    if (g.status === 'bezorgd') return;
...
function queueMessage(id, order) {
  ...
  if (before === 1) return 'Nog 1 bestelling voor jou.';
  return `Nog ${before} bestellingen voor jou.`;
}
