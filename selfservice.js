...
function statusRank(status) {
  // De volgorde van de fases in de keuken/bar-wachtrij.
  // Een bestelling die nog in een eerdere fase staat, staat ook vóór
  // bestellingen die al verder in het proces zijn.
  return { nieuw: 0, bereiden: 1, klaar: 2, bezorgd: 3 }[status] ?? -1;
}

function orderGroups() {
  // Maak van gesplitste keuken/bar-tickets weer één wachtrij-item.
  // De tickets blijven in "Mijn bestellingen" gewoon apart zichtbaar.
  const groups = new Map();

  Object.entries(allOrders).forEach(([oid, o]) => {
    if (!o || !['nieuw', 'bereiden', 'klaar', 'bezorgd'].includes(o.status)) return;
    const gid = o.orderGroupId || oid;
    if (!groups.has(gid)) groups.set(gid, { ids: [], orders: [], tijd: o.tijd || 0 });
    const g = groups.get(gid);
    g.ids.push(oid);
    g.orders.push(o);
    g.tijd = Math.min(g.tijd || (o.tijd || 0), o.tijd || 0);
  });

  // De effectieve fase van een gesplitste bestelling is de langzaamste
  // component. Daardoor telt keuken + bar bij "klaar" pas als één bestelling
  // wanneer beide delen klaar zijn.
  groups.forEach(g => {
    const active = g.orders.filter(o => o.status !== 'bezorgd');
    g.phase = active.length
      ? active.reduce((min, o) => Math.min(min, statusRank(o.status)), 99)
      : 3;
  });
...
function positionBefore(id, order) {
  ...
  const phase = statusRank(order.status);
  const mijnGroep = order.orderGroupId || id;
  const groups = orderGroups();
  const mine = groups.get(mijnGroep);
  const mijnTijd = mine?.tijd ?? order.tijd ?? 0;

  let voorMij = 0;
  groups.forEach((g, gid) => {
    if (gid === mijnGroep) return;
    if (g.tijd >= mijnTijd) return;

    // Tel unieke bestellingen die nog in deze fase of een eerdere fase zitten.
    // Hierdoor tellen keuken + bar nooit dubbel mee. Bestellingen die al
    // verder zijn (bijv. al bezorgd) tellen uiteraard niet meer mee.
    if (g.phase <= phase) voorMij++;
  });
  return voorMij;
}
