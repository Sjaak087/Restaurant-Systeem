...
function positionBefore(id, order) {
  ...
  // In "nieuw" en "bereiden" telt elke fase normaal per bestelling.
  // In "klaar" kan één zelfservicebestelling echter uit twee tickets bestaan
  // (keuken + bar). Die tickets blijven bewust apart zichtbaar bij
  // "Mijn bestellingen", maar mogen in de bezorgwachtrij maar ÉÉN keer tellen.
  ...
  Object.entries(allOrders).forEach(([oid, o]) => {
    ...
    // Voor de klaar-/bezorgfase: tel een gedeelde keuken+bar-order als één
    // wachtrij-item. Alleen tickets die daadwerkelijk klaar zijn doen mee.
    if (phase === 'klaar') {
      if (o.status !== 'klaar') return;
    } else {
      // Voor nieuw/bereiden blijft het gedrag per fase behouden.
      if (o.status !== phase) return;
    }
