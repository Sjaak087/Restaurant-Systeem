...
function getOrderGroup(id, order) {
  const groupId = order?.orderGroupId || id;
  return Object.entries(allOrders)
    .filter(([oid, o]) => o && (o.orderGroupId || oid) === groupId)
    .map(([oid, o]) => ({ id: oid, order: o }));
}

function customerOrderData(id, order) {
  const group = getOrderGroup(id, order);
  const merged = { ...order, items: {}, itemOpties: {} };
  group.forEach(({ order: part }) => {
    Object.entries(part.items || {}).forEach(([key, amount]) => {
      merged.items[key] = (merged.items[key] || 0) + amount;
      if (part.itemOpties?.[key]) merged.itemOpties[key] = part.itemOpties[key];
    });
  });
  return merged;
}

// Een bestelling die vanuit zelfservice zowel keuken- als bar-items bevat,
// wordt intern als twee tickets opgeslagen. Voor de klant is dat echter één
// bestelling. Pas als ALLE tickets van de groep klaar zijn, is de bestelling
// voor de klant "Klaar".
function customerStatus(id, order) {
  const group = getOrderGroup(id, order);
  const statuses = group.map(x => x.order.status);

  if (statuses.length && statuses.every(s => s === 'bezorgd')) return 'bezorgd';
  if (statuses.length && statuses.every(s => s === 'klaar' || s === 'bezorgd')) return 'klaar';
  if (statuses.some(s => s === 'bereiden' || s === 'klaar')) return 'bereiden';
  if (statuses.length && statuses.every(s => s === 'nieuw')) return 'nieuw';
  return statuses[0] || order.status;
}

function statusInfo(order, id) {
  const status = customerStatus(id, order);
...
function isWaitingForService(order, id) {
  const status = customerStatus(id, order);
  return !!order && ['nieuw', 'bereiden', 'klaar'].includes(status);
}

function positionBefore(id, order) {
  if (!isWaitingForService(order, id)) return 0;

  // Tel klantbestellingen, niet de losse keuken- en bar-tickets.
  // Een gesplitste bestelling wordt pas in de "klaar"-fase meegeteld als
  // zowel het keuken- als het bar-ticket klaar is.
  const phase = customerStatus(id, order);
  const mijnGroep = order.orderGroupId || id;
  const groepenTijd = new Map();

  const alleGroepen = new Map();
  Object.entries(allOrders).forEach(([oid, o]) => {
    if (!o) return;
    const gid = o.orderGroupId || oid;
    if (!alleGroepen.has(gid)) alleGroepen.set(gid, []);
    alleGroepen.get(gid).push([oid, o]);
  });

  alleGroepen.forEach((tickets, gid) => {
    const representativeId = tickets[0][0];
    const representative = tickets[0][1];
    const groupOrder = { id: representativeId, order: representative };
    const groupStatus = customerStatus(representativeId, representative);
    if (groupStatus !== phase) return;

    const tijd = Math.min(...tickets.map(([, o]) => o.tijd || 0));
    groepenTijd.set(gid, tijd);
  });

  const mijnTijd = Math.min(...getOrderGroup(id, order).map(x => x.order.tijd || 0));
  let voorMij = 0;
...
function queueMessage(id, order) {
  if (!isWaitingForService(order, id)) return '';
...
function renderMine() {
  ...
  // Toon een gesplitste keuken/bar-bestelling als één klantbestelling.
  const groups = new Map();
  Object.entries(myOrders).forEach(([id, order]) => {
    const gid = order.orderGroupId || id;
    if (!groups.has(gid)) groups.set(gid, []);
    groups.get(gid).push([id, order]);
  });
  const entries = Array.from(groups.values())
    .map(group => {
      group.sort((a,b)=>(a[1].tijd||0)-(b[1].tijd||0));
      return group[0];
    })
    .sort((a,b)=>(b[1].tijd||0)-(a[1].tijd||0));
  ...
  entries.forEach(([id,o]) => {
    ...
    card.innerHTML = `
      ...
      <div class="selfservice-items">${esc(itemText(customerOrderData(id, o)))}</div>
