...
// Bepaal de gezamenlijke fase van een bestelling. Een bestelling die door
// meerdere afdelingen (bijv. keuken + bar) is opgesplitst, blijft één order
// voor de wachtrij. De gezamenlijke fase is de minst ver gevorderde fase van
// alle tickets. Daardoor komt een order pas bij "Klaar" als zowel keuken als
// bar klaar zijn.
function getOrderGroups() {
  ...
  Object.entries(allOrders).forEach(([oid, o]) => {
    ...
    if ((o.tijd || 0) < g.tijd || !g.tijd) g.tijd = o.tijd || 0;
  });
  ...
  groups.forEach(g => {
    ...
    // Laagste fase bepaalt de gezamenlijke voortgang.
...
function positionBefore(id, order) {
  ...
  const phaseRank = { nieuw: 0, bereiden: 1, klaar: 2, bezorgd: 3 };
  const mijnFase = mijn.status;
  const mijnTijd = mijn.tijd || order.tijd || 0;
  let voorMij = 0;

  groups.forEach((g, gid) => {
    if (gid === mijnGroep) return;
    // Alleen volledige orders in dezelfde gezamenlijke fase tellen mee.
    if (g.status !== mijnFase) return;
