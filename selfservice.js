...
// De wachtrij is per fase/tab: tel simpelweg hoeveel losse orderkaarten
// met dezelfde fase vóór deze kaart staan. Keuken en bar blijven hierbij
// bewust aparte bestellingen, precies zoals ze in "Mijn bestellingen" staan.
function positionBefore(id, order) {
  if (!isWaitingForService(order)) return 0;

  const mijnTijd = Number(order.tijd) || 0;
  let voorMij = 0;

  Object.entries(allOrders).forEach(([oid, other]) => {
    if (!other || oid === id) return;
    if (other.deviceId !== deviceId) return;
    if (other.status !== order.status) return;

    const andereTijd = Number(other.tijd) || 0;
    if (andereTijd < mijnTijd || (andereTijd === mijnTijd && oid < id)) {
      voorMij++;
    }
  });
