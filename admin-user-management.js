...
(function () {
  ...
  document.getElementById('admin-user-timeout-confirm').onclick=async()=>{ const err=document.getElementById('admin-user-timeout-error'); const value=document.getElementById('admin-user-timeout-input').value; const ts=value?new Date(value).getTime():NaN; if(!selectedUserId||!Number.isFinite(ts)||ts<=Date.now()){err.textContent='Kies een tijdstip in de toekomst.';return;} const b=document.getElementById('admin-user-timeout-confirm');b.disabled=true;try{await db.ref('users/'+selectedUserId).update({timeoutUntil:ts,banned:false});closeModal('modal-admin-user-timeout');}catch(e){console.error(e);err.textContent='Opslaan mislukt.';}finally{b.disabled=false;}};
